...
(function ($) {
  ...
  $.ajax({
    type: "GET",
    url: "http://localhost:8012/healthapp/dashboard/patient/device.php",
    data: formData,
    crossDomain: true,
    dataType: "json",
    encode: true,
...
function startConnect(dev_id) {
  ...
  let act =
    "Connecting to: " + host + " on port: " + port + ", with Node ID - " + ID;
...
function onConnect() {
  ...
  topic =
    "data/patient/" + sessionStorage.getItem("user") + "/med/" + ID + "/all";
  //topic = "#";
  // Print output for the user in the messages div
